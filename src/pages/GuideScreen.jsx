  ...
  return (
    <div className="flex flex-col h-full text-ink">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <h2 className="text-lg font-extrabold tracking-tight">사용 가이드</h2>
      ...
      <div className="flex-1 overflow-y-auto scrollbar-hide space-y-3 pb-8">
        {GUIDE_ITEMS.map((item, idx) => {
          ...
          return (
            <div 
              key={item.num} 
              className={`border rounded-2xl overflow-hidden bg-surface card-shadow transition-all ${
                isOpen ? 'border-teal' : 'border-line'
              }`}
            >
              {/* 타이틀 버튼 */}
              <div 
                className="p-4 flex items-center gap-3 cursor-pointer select-none hover:bg-app-bg transition-colors"
              ...
              >
                ...
                <h4 className="flex-1 text-sm font-bold text-ink">
                  {item.title}
                </h4>
                <ChevronDown 
                  size={16} 
              ...
              <div className={`accordion-content ${isOpen ? 'open' : ''}`}>
                <div className="px-4 pb-4 pl-13 text-xs text-ink-2 leading-relaxed border-t border-line">
                  {item.desc}
                </div>
              </div>
            </div>
