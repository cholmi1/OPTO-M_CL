import { useState } from 'react'
import { GlassCard } from '../components/ui/GlassCard'
import { Calendar, Search, ArrowLeft, Download } from 'lucide-react'

export default function SendScreen({ onNavigate, records }) {
  const [startDate, setStartDate] = useState('2026-05-31')
  const [endDate, setEndDate] = useState('2026-06-29')
  const [filteredRecords, setFilteredRecords] = useState([])
  const [hasSearched, setHasSearched] = useState(false)

  const handleSearch = () => {
    const start = new Date(startDate)
    const end = new Date(endDate)
    // 시간 범위 포함을 위해 end 날짜의 시간을 23:59:59로 설정
    end.setHours(23, 59, 59, 999)

    const filtered = records.filter(r => {
      const recordDate = new Date(r.date)
      return recordDate >= start && recordDate <= end
    })

    setFilteredRecords(filtered)
    setHasSearched(true)
  }

  const handleDownloadCSV = () => {
    if (filteredRecords.length === 0) {
      alert('다운로드할 데이터가 없습니다. 먼저 기간을 검색해주세요.')
      return
    }

    // 한글 깨짐 방지 BOM 추가
    let csvContent = '\uFEFF'
    csvContent += '날짜,요일,시간,안압(mmHg),방향,기기명,안약명,점안여부\n'

    filteredRecords.forEach(r => {
      const eyeText = r.eye === 'right' ? '우안(OD)' : '좌안(OS)'
      const takenText = r.taken ? '완료' : '미완료'
      csvContent += `${r.date},${r.dayOfWeek},${r.time},${r.value},${eyeText},${r.device},${r.medicine},${takenText}\n`
    })

    const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' })
    const url = URL.createObjectURL(blob)
    const link = document.createElement('a')
    link.href = url
    link.setAttribute('download', `tono_i_records_${startDate}_to_${endDate}.csv`)
    document.body.appendChild(link)
    link.click()
    document.body.removeChild(link)
  }

  return (
    <div className="flex flex-col h-full text-ink">
      {/* 헤더 */}
      <div className="flex items-center justify-between mb-6">
        <button 
          className="p-2 bg-surface hover:bg-teal-soft rounded-full border border-line cursor-pointer"
          onClick={() => onNavigate('menu')}
        >
          <ArrowLeft size={18} className="text-ink-2" />
        </button>
        <h2 className="text-lg font-extrabold tracking-tight">기록 전송</h2>
        <div className="w-9"></div> {/* 좌측 버튼과의 균형용 공백 */}
      </div>

      {/* 기간선택 카드 */}
      <GlassCard className="p-5 mb-6">
        <div className="flex items-center gap-2 text-teal-deep mb-4 text-sm font-bold">
          <Calendar size={16} />
          <span>기간선택</span>
        </div>

        <div className="flex flex-col gap-3 mb-5">
          <div className="flex flex-col gap-1">
            <span className="text-xs text-ink-2 font-semibold">시작일</span>
            <input 
              type="date" 
              value={startDate}
              onChange={(e) => setStartDate(e.target.value)}
              className="bg-app-bg border border-line rounded-xl px-4 py-2 text-sm text-ink font-semibold outline-none focus:border-teal"
            />
          </div>
          <div className="flex flex-col gap-1">
            <span className="text-xs text-ink-2 font-semibold">종료일</span>
            <input 
              type="date" 
              value={endDate}
              onChange={(e) => setEndDate(e.target.value)}
              className="bg-app-bg border border-line rounded-xl px-4 py-2 text-sm text-ink font-semibold outline-none focus:border-teal"
            />
          </div>
        </div>

        <button 
          className="w-full bg-teal hover:bg-teal-deep text-white font-bold py-3 rounded-xl transition-all flex items-center justify-center gap-2 cursor-pointer shadow-md shadow-teal/20 active:scale-98"
          onClick={handleSearch}
        >
          <Search size={16} />
          <span>검 색</span>
        </button>
      </GlassCard>

      {/* 검색 결과 목록 */}
      {hasSearched && (
        <div className="flex-1 flex flex-col min-h-0">
          <div className="flex items-center justify-between mb-3 px-1">
            <span className="text-xs text-ink-2 font-semibold">검색 결과: {filteredRecords.length}건</span>
            {filteredRecords.length > 0 && (
              <button 
                className="text-xs text-teal-deep hover:text-teal flex items-center gap-1 font-bold cursor-pointer"
                onClick={handleDownloadCSV}
              >
                <Download size={14} />
                <span>CSV 다운로드</span>
              </button>
            )}
          </div>

          <div className="flex-1 overflow-y-auto scrollbar-hide space-y-3 pb-6">
            {filteredRecords.length === 0 ? (
              <div className="text-center py-8 text-sm text-ink-3">
                해당 기간의 측정 기록이 없습니다.
              </div>
            ) : (
              filteredRecords.map(r => (
                <div 
                  key={r.id}
                  className="p-4 bg-surface border border-line rounded-xl flex justify-between items-center text-xs card-shadow"
                >
                  <div className="space-y-1">
                    <div className="font-bold text-ink num">
                      {r.date} ({r.dayOfWeek.slice(0, 1)})
                    </div>
                    <div className="text-ink-3 text-[10px] num">
                      {r.time} | {r.medicine}
                    </div>
                  </div>
                  <div className="text-right">
                    <div className="text-sm font-black text-ink num">
                      {r.value} mmHg
                    </div>
                    <div className="text-[10px] text-teal-deep font-bold">
                      {r.eye === 'right' ? '우안' : '좌안'}
                    </div>
                  </div>
                </div>
              ))
            )}
          </div>
        </div>
      )}
    </div>
  )
}
