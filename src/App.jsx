...
import IdropScreen from './pages/IdropScreen'
...
export default function App() {
  ...
  const renderScreen = () => {
    switch (currentScreen) {
      ...
      case 'report':
        return <ReportScreen onNavigate={setCurrentScreen} records={records} />
      case 'my':
        return <MyScreen onNavigate={setCurrentScreen} isBluetoothConnected={isBluetoothConnected} />
      case 'cover':
        return <CoverScreen onNavigate={setCurrentScreen} />
      case 'login':
        return <LoginScreen onNavigate={setCurrentScreen} />
      case 'idrop':
        return <IdropScreen onNavigate={setCurrentScreen} />
      case 'bio':
        return <BioScreen onNavigate={setCurrentScreen} />
      case 'survey':
        return <SurveyScreen onNavigate={setCurrentScreen} />
      case 'menu': 
        return (
          <MenuScreen 
            onNavigate={setCurrentScreen} 
    ...
    <div className="app-container">
      ...
      <div className="simulator-section">
        <div className={`phone-mockup ${isAlarmPopupOpen ? 'shake-active' : ''}`}>
          ...
          <div className="phone-screen">
            ...
            {/* 하단 탭 바 (메인 4개 탭에서만 노출) */}
            {['home', 'record', 'report', 'my'].includes(currentScreen) && (
              <TabBar current={currentScreen} onNavigate={setCurrentScreen} />
            )}
            ...
            {isBluetoothConnecting && (
              <div className="scan-overlay-backdrop">
                <div className="scan-box">
                  ...
                  <p className="text-sm font-bold text-teal-deep">스마트 안압계 연결 중...</p>
                </div>
              </div>
            )}
            ...
            {isAlarmPopupOpen && (
              <div className="popup-backdrop">
                <div className="w-full max-w-[280px] bg-surface border border-line rounded-3xl p-6 text-center shadow-2xl rise-in">
                  <div className="w-16 h-16 bg-teal-soft rounded-full flex items-center justify-center mx-auto mb-4">
                    <Bell className="text-teal w-8 h-8 animate-bounce" />
                  </div>
                  <h2 className="text-xl font-extrabold text-ink mb-1">안압 측정 알림</h2>
                  <p className="text-xs text-teal-deep font-semibold mb-4">정기 측정 시간입니다</p>

                  <div className="bg-app-bg border border-line rounded-xl p-3 mb-5 text-left text-sm space-y-2">
                    <div className="flex justify-between">
                      <span className="text-ink-2 font-semibold">예약시간</span>
                      <span className="font-bold text-warn num">{alarmInfo.time}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-ink-2 font-semibold">측정방향</span>
                      <span className="font-bold text-ink">{alarmInfo.eye === 'right' ? '우안(OD)' : '좌안(OS)'}</span>
                    </div>
                  </div>

                  <button 
                    className="w-full bg-teal hover:bg-teal-deep text-white text-sm font-bold py-3 rounded-xl transition-colors cursor-pointer shadow-md shadow-teal/20"
                    onClick={() => {
                ...
                <div className="result-modal-card">
                  ...
                  <div className="result-form-container">
                    <div className="result-form-row">
                      ...
                      <input 
                        type="number" 
                        className="result-field-input font-bold text-teal-deep num" 
                      ...
                      <div className="flex gap-2 flex-1">
                        <button 
                          className={`flex-1 py-1 rounded text-xs font-bold cursor-pointer transition-colors ${
                            selectedRecordForEdit.eye === 'left' ? 'bg-teal text-white' : 'bg-app-bg text-ink-3 border border-line'
                          }`}
                        ...
                        <button 
                          className={`flex-1 py-1 rounded text-xs font-bold cursor-pointer transition-colors ${
                            selectedRecordForEdit.eye === 'right' ? 'bg-violet text-white' : 'bg-app-bg text-ink-3 border border-line'
                          }`}
      ...
      <div className="guide-section scrollbar-hide">
        <header className="guide-header">
          <span className="badge">INTEGRATED APP DEMO</span>
          <h1>OPTO-M 통합 녹내장 관리 솔루션</h1>
          <p>안압계 연동(TONO-i) 및 점안관리(IDROPM)를 통합한 헬스케어 프로토타입입니다. 우측 제어판과 좌측 스마트폰 시뮬레이터를 통해 복합적인 녹내장 자가 관리 시나리오를 테스트할 수 있습니다.</p>
        </header>
        ...
        <div className="guide-card">
          ...
          <div className="demo-buttons-grid">
            {/* 신규 통합 페이지 단축 버튼 */}
            <button 
              className={`demo-nav-btn ${currentScreen === 'home' ? 'active' : ''} col-span-2`}
              onClick={() => setCurrentScreen('home')}
            >
              <span className="step-num">★</span>
              <span className="step-name font-bold">홈 대시보드 (신규)</span>
            </button>
            <button 
              className={`demo-nav-btn ${currentScreen === 'report' ? 'active' : ''}`}
              onClick={() => setCurrentScreen('report')}
            >
              <span className="step-num">★</span>
              <span className="step-name font-bold">건강 리포트 (신규)</span>
            </button>
            <button 
              className={`demo-nav-btn ${currentScreen === 'my' ? 'active' : ''}`}
              onClick={() => setCurrentScreen('my')}
            >
              <span className="step-num">★</span>
              <span className="step-name font-bold">전체 탭 (신규)</span>
            </button>
            <button 
              className={`demo-nav-btn ${currentScreen === 'login' ? 'active' : ''} col-span-2`}
              onClick={() => setCurrentScreen('login')}
            >
              <span className="step-num">★</span>
              <span className="step-name font-bold">로그인 화면 (신설)</span>
            </button>
            <button 
              className={`demo-nav-btn ${currentScreen === 'cover' ? 'active' : ''} col-span-2`}
              onClick={() => setCurrentScreen('cover')}
            >
              <span className="step-num">★</span>
              <span className="step-name font-bold">통합 표지 (홈 화면)</span>
            </button>
            <button 
              className={`demo-nav-btn ${currentScreen === 'idrop' ? 'active' : ''}`}
              onClick={() => setCurrentScreen('idrop')}
            >
              <span className="step-num">★</span>
              <span className="step-name font-bold">점안관리 (IDROPM)</span>
            </button>
            <button 
              className={`demo-nav-btn ${currentScreen === 'bio' ? 'active' : ''}`}
              onClick={() => setCurrentScreen('bio')}
            >
              <span className="step-num">★</span>
              <span className="step-name font-bold">생체신호 페이지</span>
            </button>
            <button 
              className={`demo-nav-btn ${currentScreen === 'survey' ? 'active' : ''} col-span-2`}
              onClick={() => setCurrentScreen('survey')}
            >
              <span className="step-num">★</span>
              <span className="step-name font-bold">문진표 페이지 (12문항)</span>
            </button>

            <div className="col-span-2 border-t border-line my-1"></div>
            ...
            >
              ...
              <span className="step-name">안압 메인메뉴</span>
            </button>
            <button 
        ...
        <div className="guide-card">
          <h3>💡 실시간 기능 연계 팁</h3>
          <ul>
            <li><strong>로그인 후 홈 대시보드</strong>가 기본 진입 화면이며, 하단 <strong>홈/측정/리포트/전체</strong> 탭으로 어디서든 즉시 이동할 수 있습니다.</li>
            <li>홈 대시보드의 각 카드(점안·걸음·수면·심박·알람·문진)를 터치하면 해당 상세 화면으로 바로 이동합니다.</li>
